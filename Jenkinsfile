    ...
    stages {
        ...
        stage('Build and push images ') {
            steps {
                ...
                sh "sudo docker build -t muhammadatef/project-nginx:latest -t muhammadatef/project-nginx:\$(git rev-parse HEAD  ,length=6) -f ./nginx/Dockerfile ./nginx"
                sh "sudo docker push muhammadatef/project-nginx:latest" 
                sh "sudo docker push muhammadatef/project-nginx:\$(git rev-parse   ,length=6)" 

                sh "sudo docker build -t muhammadatef/project-web:latest -t muhammadatef/project-web:\$(git rev-parse   ,length=6) -f ./web/Dockerfile ./web"
                sh "sudo docker push muhammadatef/project-web:latest" 
                sh "sudo docker push muhammadatef/project-web:\$(git rev-parse   ,length=6)" 
